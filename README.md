...
- [CLI-команды](#cli-команды)
  ...
  - [add — добавить в очередь](#add--добавить-в-очередь)
  - [process — выполнить очередь](#process--выполнить-очередь)
...
Создаёт задачу в базе данных со статусом `pending`, но не запускает пайплайн. Позволяет накопить несколько задач и выполнить их позже через `process`.

```bash
ai-pipeline add <описание> --repo <путь> [опции]
```
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|:------------:|----------|
| `<описание>` | строка | да | Текстовое описание задачи |
| `--repo <путь>` | путь | да | Путь к целевому git-репозиторию |
| `--max-attempts <n>` | число | нет | Максимальное количество попыток (по умолчанию: 3) |
...
```bash
ai-pipeline add "добавь валидацию email" --repo ~/projects/my-app
# ✓ Task #1 added: добавь валидацию email

ai-pipeline add "добавь кнопку логаута" --repo ~/projects/my-app
# ✓ Task #2 added: добавь кнопку логаута
```

---

### `process` — Выполнить очередь

Берёт все pending-задачи для указанного репозитория и выполняет их последовательно (FIFO — старые первыми).

```bash
ai-pipeline process --repo <путь> [опции]
```
...
| `--auto-merge` | флаг | нет | Автоматический мерж при успехе |
| `--limit <n>` | число | нет | Максимальное количество задач (по умолчанию: все pending) |

**Пример:**

```bash
# Выполнить все pending-задачи
ai-pipeline process --repo ~/projects/my-app

# Выполнить максимум 2 задачи с автомержем
ai-pipeline process --repo ~/projects/my-app --auto-merge --limit 2
```
...
**Коды завершения:** 0 — все задачи done, 1 — хотя бы одна failed.
...
class TaskRepository {
  ...
  listPending(repoPath: string, limit?: number): TaskRecord[]  // FIFO-порядок
...
Класс `PipelineRunner` — сердце системы. Три публичных метода:

- `run()` — создаёт задачу в БД и выполняет её (для команды `run`)
- `runExisting()` — выполняет уже существующую pending-задачу (для команды `process`)
- `retry()` — перезапускает задачу в статусе `failed`

Все три делегируют приватному методу `execute()`, который реализует полный цикл:
1. Проверяет чистоту репозитория
2. Для каждой попытки:
...
3. Логирует каждое действие в `task_logs`
4. Обновляет статус задачи на каждом шагу
...
- Метод `call()` — отправляет system prompt + user prompt, возвращает текст ответа, счётчики токенов и `stopReason`
- Метод `callWithRetry()` — обёртка над `call()` + `parseJSON()` с автоматическим ретраем (до 2 попыток) при ошибке парсинга JSON. Не ретраит при `stop_reason === 'max_tokens'` (обрезанный ответ). Суммирует токены по всем попыткам
...
2. Вызывает `callWithRetry()` — автоматический ретрай при ошибке парсинга JSON
3. Валидирует структуру (наличие `files`, `commitMessage`)
4. Возвращает результат + метрики (tokensUsed, durationMs)
...
2. Вызывает `callWithRetry()` — автоматический ретрай при ошибке парсинга JSON
3. Валидирует решение (`approve`/`reject`)
...
ai-pipeline add "добавь валидацию email" --repo ~/projects/my-app
ai-pipeline add "добавь кнопку логаута" --repo ~/projects/my-app
ai-pipeline add "исправь баг с пагинацией" --repo ~/projects/my-app

# Проверить очередь
ai-pipeline tasks --status pending

# Выполнить все pending-задачи последовательно
ai-pipeline process --repo ~/projects/my-app --auto-merge

# Или только первые 2
ai-pipeline process --repo ~/projects/my-app --limit 2
```
...
### `... returned invalid JSON after N attempts`

Иногда модель возвращает ответ не в формате JSON. Пайплайн автоматически ретраит вызов API (до 2 попыток). Если все попытки провалились — задача перейдёт в `failed`. Используйте `retry` для повтора.

### `... returned truncated response (max_tokens)`

Ответ модели обрезан лимитом токенов (`max_tokens = 16384`). Ретрай в этом случае не выполняется, т.к. повтор даст тот же результат. Попробуйте упростить задачу или разбить на несколько мелких.
