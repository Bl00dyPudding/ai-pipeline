...
Мультиагентный конвейер разработки на базе Claude AI. Система из четырёх ИИ-агентов автоматизирует полный цикл: получает задачу текстом, составляет план реализации, пишет код, проводит код-ревью, запускает тесты и мержит результат.
...
Вы: "добавь валидацию email в форму регистрации"
   ...
   └──────┬───────┘
          ...
          ▼
   ┌──────────────┐
   │Planner Agent │  Claude API → анализирует кодовую базу → план реализации
   └──────┬───────┘
          │
          ▼
   ┌──────────────┐
   │ Coder Agent  │  Claude API → генерирует файлы по плану → коммит в ветку
   └──────┬───────┘
          │
          ▼
   ┌──────────────┐     ┌─── reject ──→ обратно к Planner (макс. 3 попытки)
   │Reviewer Agent│─────┤
   └──────┬───────┘     └─── approve ──→ далее
          │
          ▼
   ┌──────────────────┐  ┌─── fail ────→ обратно к Planner (макс. 3 попытки)
   │Build+Lint+Тесты  │──┤
   └──────┬───────────┘  └─── pass ────→ далее
          │
          ▼
...
Ключевая идея: задача проходит через конвейер агентов. Сначала планировщик анализирует кодовую базу и создаёт пошаговый план, затем кодер реализует его. Если ревьюер отклонил код или тесты упали — планировщик пересоставляет план с учётом фидбека, и кодер пробует снова (до 3 попыток). Каждая попытка создаёт чистую ветку от main.
...
| **Planner Agent** | Анализирует кодовую базу и создаёт пошаговый план реализации |
| **Coder Agent** | Генерирует код по плану и контексту проекта |
| **Reviewer Agent** | Ревьюит git diff без знания исходной задачи |
| **Test Runner** | Запускает билд, ESLint и тесты проекта |
...
3. Запускается цикл попыток (до `maxAttempts`):
   - **Planning**: собирается контекст репо → вызывается Planner Agent → создаётся пошаговый план (при ретрае — план пересоставляется с учётом фидбека)
   - **Coding**: вызывается Coder Agent с планом → файлы записываются → коммит в ветку `ai/task-{id}-attempt-{n}`
   - **Reviewing**: получается git diff → вызывается Reviewer Agent → при reject — сохраняется фидбек, цикл продолжается
   - **Testing**: запускается build + lint + тесты → при fail — сохраняется вывод, цикл продолжается
...
| Параметр | Описание |
|----------|----------|
| `--status <статус>` | Фильтр по статусу: `pending`, `planning`, `coding`, `reviewing`, `testing`, `done`, `failed` |
...
────────────────────────────────────────────────────────────
 Agent Logs
────────────────────────────────────────────────────────────
  [2025-01-15 10:30:03] planner/plan
    Input:  Task: добавь валидацию email
    Output: 3 steps, 2 files: Add email validation using regex pattern
    Tokens: 8200
    Duration: 5100ms
...
- Просмотр логов агентов (планировщик, кодер, ревьюер, тестер)
...
| `planning` | Planner Agent анализирует кодовую базу и составляет план |
| `coding` | Coder Agent генерирует код по плану |
| `reviewing` | Reviewer Agent проверяет diff |
| `testing` | Запущены билд, lint и тесты |
...
2. Контекст репозитория собирается **заново** (на случай если main изменился)
3. Планировщик **пересоставляет план** с учётом обратной связи (если есть):
   - Комментарии ревьюера (какие issues найдены)
   - Вывод упавшего билда/тестов/линтера
4. Кодер реализует обновлённый план
...
**Файл:** `src/agents/planner.ts`

**Роль:** Senior software architect. Получает контекст проекта + описание задачи + (опционально) обратную связь с предыдущей попытки. Анализирует кодовую базу и создаёт детальный пошаговый план реализации.

**Системный промпт:**
- Глубоко изучить существующие паттерны, конвенции и архитектуру
- Определить ВСЕ файлы, которые нужно изменить или создать
- Разбить задачу на упорядоченные шаги с зависимостями
- Отметить паттерны, которым кодер должен следовать
- Определить потенциальные риски и способы их митигации
- НЕ писать код — только планировать

**Формат ответа:**

```json
{
  "analysis": "Анализ кодовой базы и как задача в неё вписывается",
  "filesToModify": [
    {
      "path": "relative/path/to/file.ts",
      "reason": "Почему нужны изменения"
    }
  ],
  "steps": [
    {
      "step": 1,
      "description": "Что сделать на этом шаге",
      "files": ["relative/path/to/file.ts"]
    }
  ],
  "patternsToFollow": ["Описание паттерна, которому нужно следовать"],
  "risks": [
    {
      "risk": "Что может пойти не так",
      "mitigation": "Как предотвратить или обработать"
    }
  ],
  "strategy": "Общая стратегия реализации"
}
```

**Важно:**
- План пересоставляется при каждой новой попытке (если есть фидбек)
- На первой попытке или при отсутствии фидбека план создаётся один раз и переиспользуется
...
**Роль:** Senior-разработчик. Получает контекст проекта + описание задачи + план от планировщика + (опционально) обратную связь с предыдущей попытки.
...
**Роль:** Запускает билд, lint и тесты целевого проекта через `child_process`.

**Логика работы:**

1. **Определение пакетного менеджера**: ищет `pnpm-lock.yaml` → `yarn.lock` → по умолчанию `npm`
2. **Build**: если в `package.json` есть скрипт `build` — запускает `{pm} run build`. При провале — lint и тесты **не запускаются** (early return)
3. **Lint**: если в `package.json` есть скрипт `lint` — запускает `{pm} run lint`
4. **Тесты**: если в `package.json` есть скрипт `test` — запускает `{pm} run test`
5. Если скрипт не найден — этап пропускается (считается пройденным)
...
interface TestResult {
  passed: boolean;      // build И lint И тесты прошли
  buildOutput: string;  // stdout + stderr билда
...
| `status` | TEXT | Текущий статус: `pending`/`planning`/`coding`/`reviewing`/`testing`/`done`/`failed` |
...
| `agent` | TEXT | Роль агента: `planner`/`coder`/`reviewer`/`tester`/`pipeline` |
| `action` | TEXT | Действие: `plan`, `generate`, `review`, `test`, `complete` |
...
│   │   ├── planner.ts        # PlannerAgent — анализ и планирование
│   │   ├── coder.ts          # CoderAgent — генерация кода по плану
...
│   ├── test-runner/
│   │   └── executor.ts       # Запуск build + lint + тестов через child_process
│   ├── server/
...
- `TaskStatus` — 7 возможных статусов задачи (включая `planning`)
- `AgentRole` — роли агентов (`planner`, `coder`, `reviewer`, `tester`, `pipeline`)
...
- `PlannerOutput` — формат ответа планировщика (analysis, filesToModify, steps, patternsToFollow, risks, strategy)
...
2. Для каждой попытки:
   ...
   - Собирает контекст репозитория
   - Вызывает `PlannerAgent.plan()` — составляет/пересоставляет план (при наличии фидбека)
   - Вызывает `CoderAgent.generate()` с контекстом + планом + фидбеком
...
### `src/agents/planner.ts` — Агент-планировщик

Наследует `BaseAgent`. Метод `plan()`:
...
Содержит:
- `PLANNER_SYSTEM_PROMPT` — системный промпт планировщика с ролью архитектора и форматом ответа
...
- `buildPlannerUserPrompt()` — собирает пользовательский промпт из контекста + задачи + фидбека
- `buildCoderUserPrompt()` — собирает пользовательский промпт из контекста + задачи + плана + фидбека
...
- Проверяет наличие скриптов `build`, `lint` и `test` в `package.json`
- Запускает команды через `child_process.execFile` с shell mode в порядке: build → lint → test
- При провале build — lint и тесты пропускаются (early return)
...
Тест-раннер ищет скрипты `build`, `lint` и `test` в `package.json` целевого проекта. Если их нет — этапы пропускаются (считаются пройденными). При провале `build` — lint и тесты не запускаются.
